...
public class Screenshotter {
	...
	private static final String TEST_MOUSE_DATASET = "test/samples/images/Mouse/UnitTest_" + Version.currentVersion()
			+ "/Mouse.nmd";
	private static final String TEST_ROUND_DATASET = "test/samples/images/Round_with_signals/UnitTest_"
			+ Version.currentVersion() + "/Round_with_signals.nmd";
	...
	private void run() throws InterruptedException {
		...
		UserActionController uac = UserActionController.getInstance();
		...
		uac.userActionEventReceived(new UserActionEvent(this,
				UserActionEvent.IMPORT_DATASET_PREFIX + new File(TEST_ROUND_DATASET).getAbsolutePath()));
		...
		uac.userActionEventReceived(new UserActionEvent(this,
				UserActionEvent.IMPORT_DATASET_PREFIX + new File(TEST_MOUSE_DATASET).getAbsolutePath()));
		Thread.sleep(LOAD_TIME_MILLIS);
		...
		try {
			...
			for (TabPanel p : panels) {
				if (((Component) p).isShowing()) {
					...
					dps.takeScreenShots((DetailPanel) p, outputFolder,
							prefix + "_" + p.getClass().getSimpleName() + "_annotated");
				}
			}
