...
/**
 * A default implementation of the IShellResult interface. It uses
 * a builder pattern for setting values.
 * @author bms41
 * @since 1.13.3
 *
 */
public class DefaultShellResult implements IShellResult {
	...
	public DefaultShellResult setRawStandardErrors(CountType type, List<Double> errs){
		...
		switch(type){
			...
			default: target = signalRawStderrs;
			break;
		}
	 ...
	 * @param type the counting type
	 * @param chi the chi test result
	 * @param pval the chi test pvalue
	 * @return this shell result
	 */
    ...
    public DefaultShellResult(IShellResult s){
    	...
    	this
        	.setRawMeans(CountType.SIGNAL,         s.getRawMeans(CountType.SIGNAL))
...
//        	.setPixelCounts(CountType.NUCLEUS,     s.getPixelCounts(CountType.NUCLEUS))
        	...
        	.setNormalisedChiResult(CountType.NUCLEUS,       s.getNormalisedChiSquare(CountType.NUCLEUS), s.getNormalisedPValue(CountType.NUCLEUS));
