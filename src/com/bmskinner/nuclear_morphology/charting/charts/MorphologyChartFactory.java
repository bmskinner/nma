...
public class MorphologyChartFactory extends AbstractChartFactory {
            ...
            if (options.isShowMarkers()) {
                ...
                for (Tag tag : collection.getProfileCollection().getBorderTags()) {
                    ...
                    try {
                        ...
                        if (options.isNormalised()) { // set to the proportion
                                                      // of the point along the
                                                      // profile
                            // convert to a double to allow normalised
                            // positioning
    ...
    private JFreeChart makeMultiProfileChart() {
        ...
        for (int i = 0; i < iqrProfiles.size(); i++) {
            ...
            Paint profileColour = options.getDatasets().get(index).getDatasetColour() == null
                    ? ColourSelecter.getColor(i) : options.getDatasets().get(index).getDatasetColour();
        ...
        for (int j = 0; j < medianProfiles.getSeriesCount(); j++) {
            ...
            Paint profileColour = options.getDatasets().get(index).getDatasetColour() == null
                    ? ColourSelecter.getColor(j) : options.getDatasets().get(index).getDatasetColour();
    ...
    private JFreeChart makeMultiVariabilityChart() throws Exception {
        ...
        for (int j = 0; j < ds.getSeriesCount(); j++) {
            ...
            Paint profileColour = options.getDatasets().get(index).getDatasetColour() == null
                    ? ColourSelecter.getColor(index) : options.getDatasets().get(index).getDatasetColour();
    ...
    private JFreeChart makeMultiSegmentStartPositionChart(ChartOptions options) {
        ...
        for (int j = 0; j < positionDataset.getSeriesCount(); j++) {
            ...
            Paint profileColour = options.getDatasets().get(j).getDatasetColour() == null ? ColourSelecter.getColor(j)
    ...
    private JFreeChart createModalityChart(Double position, List<IAnalysisDataset> list, ProfileType type)
        ...
        for (IAnalysisDataset dataset : list) {
            ...
            Paint colour = dataset.getDatasetColour() == null ? ColourSelecter.getColor(iteration)
    ...
    public JFreeChart createModalityProfileChart() {
        ...
        for (int i = 0; i < options.getDatasets().size(); i++) {
            ...
            Paint colour = dataset.hasDatasetColour() ? dataset.getDatasetColour() : ColourSelecter.getColor(i);
    ...
    public JFreeChart createModalityPositionChart() {
        ...
        for (IAnalysisDataset dataset : options.getDatasets()) {
            ...
            // Set the text colour
            Paint colour = dataset.getDatasetColour() == null ? ColourSelecter.getColor(index)
                    : dataset.getDatasetColour();
    ...
    public JFreeChart makeKruskalWallisChart(boolean frankenNormalise) {
        ...
        try {
            ...
            firstProfileDataset = creator.createNonsegmentedMedianProfileDataset(options.firstDataset(), true,
                    options.getAlignment(), options.getTag());

            secondProfileDataset = creator.createNonsegmentedMedianProfileDataset(options.getDatasets().get(1), true,
                    options.getAlignment(), options.getTag());
        ...
        Paint colorOne = options.getDatasets().get(0).getDatasetColour() == null ? ColourSelecter.getColor(0)
        ...
        Paint colorTwo = options.getDatasets().get(1).getDatasetColour() == null ? ColourSelecter.getColor(1)
