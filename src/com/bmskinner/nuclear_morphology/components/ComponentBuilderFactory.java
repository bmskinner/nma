...
public class ComponentBuilderFactory {
	...
	/**
	 * Factory for nucleus builders. This can be initialised
	 * with rulesets and profile window proportions that are common
	 * across all nuclei to be created to simplify the building code.
	 * @author ben
	 * @since 2.0.0
	 *
	 */
	public static class NucleusBuilderFactory {
		...
		public class NucleusBuilder {			
			...
			public Nucleus build() throws ComponentCreationException {
				 ...
				 if(isOffset) {
				        IPoint offsetCoM = new FloatPoint(com.getX() - (int) roi.getXBase(), com.getY() - (int) roi.getYBase());
	...
	/**
	 * Factory for signal builders. This can be initialised
	 * with fields that are common across all signals to be 
	 * created to simplify the building code.
	 * @author ben
	 * @since 2.0.0
	 *
	 */
	public static class SignalBuilderFactory {
		...
		public class SignalBuilder {			
			...
			public INuclearSignal build() throws ComponentCreationException {				 
				 ...
				 INuclearSignal s = new DefaultNuclearSignal(roi, com, file, channel, 
						 (int) roi.getXBase(), (int) roi.getYBase(), id);
     ...
     */
    public static NucleusBuilderFactory createNucleusBuilderFactory(@NonNull RuleSetCollection rsc, double prop, double scale) {
