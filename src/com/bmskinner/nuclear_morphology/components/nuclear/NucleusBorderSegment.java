...
public class NucleusBorderSegment implements IBorderSegment {
	...
	private static final Logger LOGGER = Logger.getLogger(NucleusBorderSegment.class.getName());
    ...
    private int totalLength; // the total length of the profile that this
                             // segment is a part of

    private IBorderSegment prevSegment = null; // track the previous segment in
                                               // the profile
    private IBorderSegment nextSegment = null; // track the next segment in the
                                               // profile
    ...
    private boolean startPositionLocked = false; // allow the start index to be
                                                 // fixed
    ...
    public NucleusBorderSegment(int startIndex, int endIndex, int total, UUID id) {
        ...
        if (testLength < MINIMUM_SEGMENT_LENGTH) {
            throw new IllegalArgumentException("Cannot create segment of length " + testLength + " from " + startIndex
                    + " to " + endIndex + ": shorter than " + MINIMUM_SEGMENT_LENGTH);
        }
    ...
    // int newValue =
    // AbstractCellularComponent.wrapIndex(this.getStartIndex()+value,
    // this.getTotalLength());
    ...
    // int newValue =
    // AbstractCellularComponent.wrapIndex(this.getEndIndex()-value,
    // this.getTotalLength());
    ...
    // int newValue = AbstractCellularComponent.wrapIndex(
    // this.getStartIndex()-value, this.getTotalLength());
    // return this.update(newValue, this.getEndIndex());
    // }
    //
    // /* (non-Javadoc)
    ...
    // int newValue = AbstractCellularComponent.wrapIndex(
    // this.getEndIndex()+value, this.getTotalLength());
    // return this.update(this.getStartIndex(), newValue);
    // }
...
//    }
    ...
    public boolean update(int startIndex, int endIndex) throws SegmentUpdateException {
        ...
        if (updateAffectsThisSegment(startIndex, endIndex)) {
            ...
            if (testLength < MINIMUM_SEGMENT_LENGTH) {
                this.lastFailReason = startIndex + "-" + endIndex + ": segment length (" + testLength
                        + ") cannot be smaller than " + MINIMUM_SEGMENT_LENGTH;
                return false;
            }

            // Check that next and previous segments are not invalidated by
            // length change
            // i.e the max length increase backwards is up to the MIN_SEG_LENGTH
            // of the
            // previous segment, and the max length increase forwards is up to
            // the
            ...
            if (this.hasPrevSegment()) {
                ...
                if (prevTestLength < MINIMUM_SEGMENT_LENGTH) {
                    this.lastFailReason = startIndex + "-" + endIndex
                            + ": Previous segment length cannot be smaller than " + MINIMUM_SEGMENT_LENGTH
                            + "; would be " + this.prevSegment().getStartIndex() + "-" + startIndex + "("
                            + prevTestLength + ")";
                    return false;
                }
            }
            if (this.hasNextSegment()) {
                ...
                if (nextTestLength < MINIMUM_SEGMENT_LENGTH) {
                    this.lastFailReason = startIndex + "-" + endIndex + ": Next segment length cannot be smaller than "
                            + MINIMUM_SEGMENT_LENGTH + "; would be " + endIndex + "-" + this.nextSegment().getEndIndex()
                            + "(" + nextTestLength + ")";
                    return false;
                }
            }
            ...
            // i.e. where a start becomes greater than an end without begin part
            // of
            // an array wrap
            ...
            if (this.hasNextSegment()) {
                if (endIndex > this.nextSegment().getEndIndex()) {
                    ...
                    // if the next segment goes from not wrapping to wrapping
                    // when this segment is altered,
            ...
            // wrap in if to ensure we don't go in circles forever when testing
            // a circular profile
