 ...
 * hump and tip, hence can be oriented in two axes.
 */
...
public class RodentSpermNucleus extends SpermNucleus {
	...
	private static final Logger LOGGER = Logger.getLogger(RodentSpermNucleus.class.getName());
    ...
    private double getHookOrBodyLength(boolean useHook) throws UnavailableBorderTagException {
        ...
        if (hasStatistic(PlottableStatistic.HOOK_LENGTH) || hasStatistic(PlottableStatistic.BODY_WIDTH)) {
            ...
            if (getStatistic(PlottableStatistic.HOOK_LENGTH) == -1
                    || getStatistic(PlottableStatistic.BODY_WIDTH) == -1) {
                calculateHookAndBodyLength();
            }
        ...
        stat = stat == -2d ? 0 : stat; // -2 is the error code when TV and BV
    ...
    private void calculateHookAndBodyLength() throws UnavailableBorderTagException {
        ...
        if (testNucleus != null && testNucleus.hasBorderTag(Tag.TOP_VERTICAL)
             ...
             */
            // double vertX =
            // testNucleus.getBorderTag(BorderTag.TOP_VERTICAL).getX();
            ...
            /*
             * To determine if the point is hook or hump, take the X position of
             * the tip. This must lie on the hook side of the vertX
             */
    ...
    public void findPointsAroundBorder() {
        ...
        try {
             ...
             * Perform a 5win average smoothing of the deltas Count the number
             * of consecutive >1 degree blocks Wide block far from tip = tail
             */
            ...
            // consensusTailIndex = this.getPositionBetween(consensusTail,
            // spermTail1);
    ...
    public Nucleus getVerticallyRotatedNucleus() {
         ...
         * pointing left. If not, flip thw nucleus
         */
    ...
    public IBorderPoint findTailPointFromMinima() throws UnavailableProfileTypeException {
        ...
        IBorderPoint tail = this.getBorderTag(Tag.REFERENCE_POINT); // start at
                                                                    // tip, move
                                                                    // round
        ...
        for (int i = 0; i < array.size(); i++) {
            if (array.get(i) == true) {
                ...
                double distanceBetweenEnds = this.getBorderTag(Tag.REFERENCE_POINT).getLengthTo(getBorderPoint(i));
