...
public class RoundNucleus extends AbstractCellularComponent implements Nucleus {
	...
	private static final Logger LOGGER = Logger.getLogger(RoundNucleus.class.getName());
    ...
    protected double angleWindowProportion;  // The proportion of the perimeter
                                             // to use for profiling
    protected int    angleProfileWindowSize; // the chosen window size for the
                                             // nucleus based on proportion

    protected double pathLength; // the angle profile path length - measures
                                 // wibbliness in border

    protected Map<ProfileType, ISegmentedProfile> profileMap = new HashMap<ProfileType, ISegmentedProfile>();

    protected List<NucleusBorderSegment> segmentList = new ArrayList<NucleusBorderSegment>(0); // expansion
                                                                                               // for
                                                                                               // e.g
                                                                                               // acrosome

    protected Map<Tag, Integer> borderTags = new HashMap<Tag, Integer>(0); // to
                                                                           // replace
                                                                           // borderPointsOfInterest;
                                                                           // <tag,
                                                                           // index>

    protected Map<String, Integer> segmentTags = new HashMap<String, Integer>(0);

    protected String outputFolder; // the top-level path in which to store
                                   // outputs; has analysis date e.g.
                                   // /Testing/2015-11-24_10:00:00
    ...
    public RoundNucleus(Roi roi, File file, int number, int[] position) { // construct
                                                                          // from
                                                                          // an
                                                                          // roi
        super(roi);
    ...
    public void moveCentreOfMass(IPoint centreOfMass) {
        ...
        for (UUID id : signalCollection.getSignalGroupIds()) {
            ...
            signalCollection.getSignals(id).parallelStream().forEach(s -> {
                ...
                // log(this.getNameAndNumber()+": Offsetting signal -
                // "+xOffset+", "+yOffset);
    ...
    protected double calculateStatistic(PlottableStatistic stat) {
        ...
        if (stat instanceof NucleusStatistic) {
            ...
            throw new IllegalArgumentException(
                    "Statistic type inappropriate for nucleus: " + stat.getClass().getName());
        }
     ...
     * BottomVertical points have been set, these will be used. Otherwise, the
     * given point is moved to directly below the CoM
     * 
     * @param point
     *            the point to put at the bottom. Overridden if TOP_ and BOTTOM_
     *            are set
     * @return
     * @throws Exception
     */
    ...
    public double getAspectRatio() {
        try {
            ...
            // return
            // this.getBoundingRectangle(BorderTag.ORIENTATION_POINT).getHeight()
            // /
            // this.getBoundingRectangle(BorderTag.ORIENTATION_POINT).getWidth();
        } catch (Exception e) {
            return 0;
        }
    }
    ...
    public void updateSourceFolder(File newFolder) {
        ...
        if (newFile.exists()) {
            ...
            // this.setNucleusFolder(new
            // File(this.getOutputFolder().getAbsolutePath()+File.separator+this.getImageNameWithoutExtension()));
        } else {
            throw new IllegalArgumentException(
    ...
    public void setProfile(ProfileType type, Tag tag, ISegmentedProfile p) throws UnavailableBorderTagException {
        ...
        // throw new UnavailableBorderTagException("Tag "+tag+" is not
        // present");
        // }
        if (segsLocked) {
            return;
    ...
    public int hashCode() {
        ...
        if (profileMap != null) {
            for (Map.Entry<ProfileType, ISegmentedProfile> entry : profileMap.entrySet()) {
                ...
                result = prime * result + ((entry == null) ? 0 : entry.getValue().hashCode());
            }
        }

        result = prime * result + ((segmentList == null) ? 0 : segmentList.hashCode());
        result = prime * result + ((segmentTags == null) ? 0 : segmentTags.hashCode());
