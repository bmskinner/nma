...
//		return difference;
	 ...
	 * Note - array lengths are not checked. TODO: Normalise to the length of the profiles
	 * @param arr1
	 * @param arr2
	 * @return
	 */
	private static double squareDifference(float[] arr1, float[] arr2){
		...
		for(int j=0; j<arr1.length; j++){ // for each point round the array

			difference += Math.pow(arr1[j] - arr2[j], 2); // square difference - highlights extremes
		}
    ...
    private static float[] interpolate(float[] array, int length){
        ...
        for (int i=0; i<length; i++) {
            ...
            float fraction = ( (float)i / (float)length); // get the fractional index position needed
    ...
    private static float getInterpolatedValue(float[] array, float fraction){
      ...
      if(indexLower==array.length){ // only wrap possible if fraction is range 0-1
          indexLower = 0;
      }
      
      int indexHigher  = indexLower+1;
      if(indexHigher==array.length){ // only wrap possible if fraction is range 0-1
          indexHigher = 0;
      }
...
//			tmp = tmp2;
	...
	public BooleanProfile getLocalMinima(int windowSize){
		...
		for (int i=0; i<array.length; i++) { // for each position in sperm
			...
			for(int j=0;j<prevValues.length;j++){
				...
				int prev_i = CellularComponent.wrapIndex( i-(j+1)  , this.size() ); // the index j+1 before i
				int next_i = CellularComponent.wrapIndex( i+(j+1)  , this.size() ); // the index j+1 after i
	...
	public IProfile differentiate(){
		...
		for (int i=0; i<array.length; i++) { // for each position in sperm
			...
			int prev_i = CellularComponent.wrapIndex( i-1  , this.size() ); // the index before
			int next_i = CellularComponent.wrapIndex( i+1  , this.size() ); // the index after
