...
public interface Imageable {
	 ...
	 * {@link #getPosition()} {@link #X_BASE} and {@link #Y_BASE}
	...
	static IPoint translateCoordinateToComponentImage(IPoint point, Imageable template, Imageable target){
		...
		/*
		 * When using the component images, there is both a cropping of the source image, and the addition
		 * of a buffer to each side (#COMPONENT_BUFFER) so that the edges of the object are not obscured.
		 * This means these must be corrected for when translating coordinates between component images.
		 * 
		 *               30
		 *       _Template____________
		 *       |                   |
		 *       |     Target        |
		 *       |     _____         |
		 *       |    |    |         |
		 ...
		 * 
		 * 
		 *  In the template image, point X is at 30, 20. In the target image, X is at 5, 5.
		 *  The template x-coordinate must be adjusted by -25 (the target x - template x) 
		 * 
		 */
...
//		
//		int xOffset = (int) cyCom.getX() + CellularComponent.COMPONENT_BUFFER;
//		int yOffset = (int) cyCom.getY() + CellularComponent.COMPONENT_BUFFER;
	...
	static IPoint translateCoordinateToSourceImage(IPoint point, Imageable template){
		 ...
		 * When using the component images, there is both a cropping of the source image, and the addition
		 * of a buffer to each side (#COMPONENT_BUFFER) so that the edges of the object are not obscured.
		 * These must be corrected for when translating coordinates between component images.
		 * 
		 *               300
		 *        ___________________
		 *       |                   |   Source Image
		 ...
		 *             295
		 * 
		 *  In the template image, point X is at 15, 15. This includes the COMPONENT_BUFFER, so 
		 *  the position within the template object is 15 - COMPONENT_BUFFER (5, 5).
		 *  
		 *   This position must be then be offset by the X_BASE and Y_BASE of the template.
		 *   
		 *   The template base is at (295, 195). Applying the offset to point X gives a final
		 *   position of (300, 200)
		 * 
		 */
	...
	static IPoint translateCoordinateToComponentImage(IPoint point, Imageable template){
		 ...
		 *             295
		 * 
		 *  In the template source image, point X is at (300,200). 
		 *  To get the point in the component image, we need to subtract the 
		 *  template base coordinates (295, 195), to give the position in the 
		 *  component (5,5). 
		 *  
