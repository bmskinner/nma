...
//    	LOGGER.fine("Ofsetting segment "+uuid+" by "+offset+" to start at "+CellularComponent.wrapIndex(startIndex+offset, totalLength));
    ...
    private boolean canUpdateSegment(int startIndex, int endIndex) throws SegmentUpdateException {
        ...
        if (this.hasPrevSegment()) {
        	...
        	if (this.prevSegment().getStartIndex() > startIndex) {
                ...
                if (prevSegment.wraps(startIndex, endIndex) && !IProfileSegment.contains(startIndex, endIndex, 0, totalLength))
        ...
        if (this.hasNextSegment()) {
            ...
            if (endIndex > nextSegment.getEndIndex()) {
                ...
                // another wrapping test - if the new positions induce a wrap,
                // the segment should contain 0
                if (nextSegment.wraps(startIndex, endIndex) && !IProfileSegment.contains(startIndex, endIndex, 0, totalLength)) {
                	throw new SegmentUpdateException(String.format("Segment would convert to wrapping but does not contain zero"));
                }
            }
        }
    ...
    public boolean update(int startIndex, int endIndex) throws SegmentUpdateException {
        ...
        /*
         * We used to remove any merge sources here because as segments repeatedly
         * update, we can't be sure the boundaries will stay in sync for un-merging
         * over repeated updates. If the merged segment shrinks, the merge sources may
         * end up too small to be a valid segment - what happens if a merge source
         * becomes 0 length?
         * 
         * TODO:
         * This needs to be looked at more robustly, and for now the merge sources 
         * are being left in the hope that these problems don't actually occur very
         * often given profile sizes.
         * 
         * We assume that the merge source boundary remains the same, and update each
         * source to the overall merged segment boundary. This will recursively update 
         * all merge sources.
         * 
         */
        if(hasMergeSources()) {
