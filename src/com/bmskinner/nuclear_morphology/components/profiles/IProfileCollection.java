...
/**
 * Interface for profiles within a cell collection. Handles
 * generation of median profiles from individual nuclei.
 * @author ben
 *
 */
public interface IProfileCollection extends XmlSerializable {
    ...
    /** If a profile is created without segments, this is the ID of the 
     * default segment spanning the entire profile. */
    ...
    /**
     * Discard any cached data, and calculate all median and other profiles
     * from the cells in the collection.
     ...
     */
    int getLandmarkIndex(@NonNull Landmark landmark) throws MissingLandmarkException;
    ...
    /**
     * Get the proportion of the landmark along the profile, zeroed on the reference point
     * 
     * @param landmark the landmark to find
     ...
     * @throws MissingProfileException
     */
    IProfile getIQRProfile(@NonNull ProfileType type, @NonNull Landmark landmark)
            throws MissingLandmarkException, ProfileException, MissingProfileException;
     ...
     * @return the profile
     * @throws ProfileException
     * @throws MissingLandmarkException when the landmark is not present as an offset
     ...
     */
    ISegmentedProfile getSegmentedProfile(@NonNull ProfileType type, @NonNull Landmark landmark, int quartile)
            throws MissingLandmarkException, ProfileException, MissingProfileException;  
     ...
     * segment in the list will contain the landmark at any index except the end index. The
     * segments will have their positions offset such that the landmark is at the zero index
     * of the underlying profile. 
     * 
     * @param landmark the landmark to offset by
     * @return a copy of the segments in the profile, offset to start at the landmark
     * @throws ProfileException
     */
    List<IProfileSegment> getSegments(@NonNull Landmark landmark) throws MissingLandmarkException, ProfileException;

    /**
     * Get the IDs of the segments in this collection, ordered by position from the RP
     * 
     * @return
     */
     ...
     * @throws ProfileException
     */
    IProfileSegment getSegmentAt(@NonNull Landmark landmark, int position) throws MissingLandmarkException, ProfileException;
     ...
     * @throws MissingLandmarkException 
     */
    IProfileSegment getSegmentContaining(@NonNull Landmark landmark) throws ProfileException, MissingLandmarkException;

    /**
     * Set the index for a given landmark, with RP at index zero. Setting the 
     * index of the RP will have no effect since the RP is always index zero.
     * 
