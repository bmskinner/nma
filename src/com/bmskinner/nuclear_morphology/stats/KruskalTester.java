...
public class KruskalTester implements Loggable, SignificanceTest {
        ...
        try {
            ...
            for (int i = 0; i < sampleNumber; i++) {
                ...
                try {
                    ...
                    // Perform Bonferroni correction with hard upper limit of
                    // p=1
    ...
    /**
     * Run a Kruskal-Wallis test at each position along the normalised angle
     * profiles for the two given datasets, starting at the given tag.
     * 
     * This compares profile one to frankenprofile for two, using the segment
     * pattern in one
     * 
     * @param one
     * @param two
    ...
    public IProfile testCollectionGetFrankenPValues(ChartOptions options) {
        ...
        try {
            ...
            /*
             * Make a copy of dataset two, so the nuclei segments are not
             * overwritten
             */
            IAnalysisDataset copyOfTwo = two.duplicate();

            /*
             * Ensure that the profile collections have the same lengths in each
             * collection
             */
             ...
             * 
             * Create a new ProfileCollection based on the segments from dataset
             * one
             */
            IProfileCollection pc = one.getCollection().getProfileCollection();

            /*
             * Create a segmenter just to access the segmnet fitter. Do not
            ...
            IProfileCollection frankenCollection = copyOfTwo.getCollection().getProfileCollection();
            frankenCollection.createProfileAggregate(copyOfTwo.getCollection(),
                    one.getCollection().getMedianArrayLength());

            /*
             * This returns to the Kruskal test above, but using the franken
             * profiles
             */
