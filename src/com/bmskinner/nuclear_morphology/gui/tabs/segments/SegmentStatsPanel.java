...
public class SegmentStatsPanel extends TableDetailPanel {
	...
	private TableOptions makeOptions() {
		return new TableOptionsBuilder().setDatasets(getDatasets()).setScale(GlobalOptions.getInstance().getScale())
				.setSwatch(GlobalOptions.getInstance().getSwatch()).setTarget(table)
				.setColumnRenderer(TableOptions.ALL_EXCEPT_FIRST_COLUMN, new SegmentTableCellRenderer()).build();
	}
