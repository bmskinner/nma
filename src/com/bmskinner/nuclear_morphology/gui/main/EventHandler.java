...
public class EventHandler implements Loggable, SignalChangeListener, DatasetEventListener, InterfaceEventListener {
	...
	private class ActionFactory {
		...
		public ActionFactory(){
			...
			selectedDataset =  mw.getPopulationsPanel().getSelectedDatasets().isEmpty() 
					? null 
		...
		public Runnable create(final DatasetEvent event){
			...
			if(event.method().equals(DatasetEvent.COPY_PROFILE_SEGMENTATION)){
				...
				return new RunSegmentationAction(selectedDatasets, source, SingleDatasetResultAction.ADD_POPULATION, mw);
			}
	...
	public void signalChangeReceived(final SignalChangeEvent event) {
		...
		final IAnalysisDataset selectedDataset = mw.getPopulationsPanel().getSelectedDatasets().isEmpty() 
				? null 
				: mw.getPopulationsPanel().getSelectedDatasets().get(0);
		...
		if(event.type().equals("CurateCollectionAction")){
				...
				CellCollectionOverviewDialog d = new CellCollectionOverviewDialog(mw.getPopulationsPanel().getSelectedDatasets().get(0));
...
//		}
	...
	public void interfaceEventReceived(final InterfaceEvent event) {
		...
		case REFRESH_POPULATIONS:
			mw.getPopulationsPanel().update(selected); // ensure all child datasets are included
			break;
	...
	private void refoldConsensus(final IAnalysisDataset dataset){
		...
		Runnable r = () -> {
			...
			for(TabPanel p : mw.getTabPanels()){
				if(p instanceof SegmentsDetailPanel || p instanceof NuclearStatisticsPanel || p instanceof SignalsDetailPanel || p instanceof ConsensusNucleusPanel){
