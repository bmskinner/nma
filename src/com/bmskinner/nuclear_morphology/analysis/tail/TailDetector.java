...
public class TailDetector extends Detector {
    ...
    public TailDetector(ICannyOptions options, int channel) {
        this.options = options;
        this.channel = channel;
    }

    /**
     * Given a file, try to find a sperm tail in the given channel. The nucleus
     * is used to orient the tail, and separate from other tails in the image
     * 
     * @param f
     *            the file
     * @param channel
     *            the channel with the tail signal
     * @param n
     *            the nucleus to which the tail should attach
    ...
    public List<Flagellum> detectTail(File tubulinFile, Nucleus n) throws Exception {
        ...
        // the stack in the ImageStack must be converted from the given rgb
        // channel
        ...
        if (checkDimensions(stack, n)) {
            ...
            // prune the shortest branches of the stack, ready for graph
            // creation
            // ImageStack prunedStack = pruneStack(skeletonStack);

            // detect the skeletons as graph objects, and prune shortest
            // branches
     ...
     * 
     * @param stack
     *            the stack to skeletonise
     * @param objects
     *            the list of rois to skeletonise
     * @return a new stack
     * @throws Exception
     */
    private ImageStack skeletoniseStack(ImageStack stack, Map<Roi, StatsMap> objects) throws Exception {
        ...
        // IMutableAnalysisOptions options =
        // OptionsFactory.makeAnalysisOptions();
        // IMutableDetectionOptions op = new
        // DefaultTailDetectionOptions((File)null);
        ...
        // create a copy of the filled processor and convert to a binary
        // processor
     ...
     */
    // private static List<Edge> findNextLongestBranch(Vertex v, List<Edge>
    // list){
    //
    // double maxLength = 0;
     ...
     * 
     * @param borderRois
     *            the tail borders in the image
     * @param skeletonStack
     *            the imagestack with skeletons
     * @param stackNumber
     *            the position in the stack of the skeleton
     * @return a list of skeletons as Polyline rois
     */
    // private static List<Roi> getSkeletons(List<Roi> borderRois, ImageStack
    // skeletonStack, int stackNumber){
    //
    ...
    // List<XYPoint> ends = findSkeletonEndpoints(borderRoi, skeletonStack,
    // stackNumber);
    //
    ...
    private ImageStack pruneStack(ImageStack stack) {
        ...
        // ImagePlus labelledImage = new ImagePlus("pruned image",
        // labelledStack.getProcessor(1));
        // labelledImage.show();
     ...
     * 
     * @param graphs
     *            the AnalyzeSkeleton_ graphs containing skeletons
    ...
    /**
     * From a list of skeleton rois, get only those greater than or equal to the
     * given length
     * 
     * @param skeletons
     *            the roi list
     ...
     */
    // private static List<Roi> filterSkeletons(List<Roi> skeletons, int
    // minLength){
    // List<Roi> result = new ArrayList<Roi>(0);
    //
    // for(Roi r : skeletons){
     ...
     * sperm tail
     * 
     * @param usableSkeletons
     *            the list of skeleton rois
     * @param borderRois
     *            the list of tail border rois
     * @param n
     *            the nucleus
     * @param tubulinFile
     *            the image file with the tubulin stain
     * @param channel
    ...
    private List<Flagellum> getTailsMatchingNucleus(List<Flagellum> tails, Nucleus n) {
        ...
        // && border.getFloatPolygon().contains(xpoints[xpoints.length-1],
        ...
        for (Roi skeleton : usableSkeletons) {
            ...
            if (skeleton.getLength() > 0) {
                ...
                for (Roi border : borderRois.keySet()) {
                    ...
                    if (border.getFloatPolygon().contains(xpoints[0], ypoints[0]) && border.getFloatPolygon()
                            .contains(xpoints[xpoints.length - 1], ypoints[ypoints.length - 1])) {
                        tailBorder = border;
                    }
                }
     ...
     * 
     * @param borderPolygon
     *            the roi with the tail border
     * @param skeletonStack
     *            the ImageStack containing the skeleton
     * @param stackNumber
     *            the stack in the ImageStack with the skeleton
     * @return a list of XYPoints representing skeleton branch ends
     */
    // private static List<XYPoint> findSkeletonEndpoints(Roi borderPolygon,
    // ImageStack skeletonStack, int stackNumber){
    //
    ...
    //
    // ByteProcessor p = (ByteProcessor)
    // skeletonStack.getProcessor(stackNumber);
    //
    ...
    /**
     * Create a polyline from a given endpoint of a skeleton. Looks for adjacent
     * white pixels, and continues until another endpoint is reached. Does not
     * work on unpruned skeletons - i.e only two endpoints are expected.
     * 
     * @param startPoint
     *            the pixel from which to begin building
     * @param skeletonStack
     *            the stack with the skeleton
     * @param stackNumber
     *            the position of the skeleton in the stack
     * @return
     */
    // private static PolygonRoi buildPolyLine(XYPoint startPoint, ImageStack
    // skeletonStack, int stackNumber){
    //
    ...
    //
    // List<XYPoint> list = getJoiningPoints(processor,
    // currentPoint.getXAsInt(), currentPoint.getYAsInt());
    //
    ...
    /**
     * Find the points in the 3x3 pixel box surrounding the given point that
     * have a value of 255 (white)
     * 
     * @param processor
     *            the image processor (binary processor)
     * @param p
     *            the point to search from
     * @return a list of white connected points
     */
    // private static List<XYPoint> getJoiningPoints(ImageProcessor processor,
    // int x, int y){
    //
    // List<XYPoint> result = new ArrayList<XYPoint>(0);
    ...
    //
    // if(processor.getPixel(i, j)==WHITE && !p.overlaps(centre)){ // ensure we
    // don't count the centre pixel
    // result.add(p);
    // }
    // }
    // }
    ...
    /**
     * Check that the dimensions of the input image are the same as the image
     ...
     *
     * @param image
     *            the ImagePlus to be analysed
     * @param closed
     *            should the detector get only closed polygons, or open lines
     * @param channel
