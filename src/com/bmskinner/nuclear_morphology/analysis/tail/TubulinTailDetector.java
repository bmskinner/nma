 ...
 * anti-tubulin. It can be consetucted as SwingWorker for GUI incorpoation, or
 * the static method run() can be called directly for scripted use. When used as
 * a SwingWorker, the reported progress will be the number of cells processed,
 * and upon completion, a "Finished" property change event is fired.
 */
public class TubulinTailDetector extends AnalysisWorker {
    ...
    protected Boolean doInBackground() {
        ...
        try {
            ...
            for (ICell c : getDataset().getCollection().getCells()) {
                ...
                fileLogger.log(Level.INFO, "Looking for tails associated with nucleus " + n.getSourceFileName() + "-"
                        + n.getNucleusNumber());
                ...
                TailDetector finder = new TailDetector(getDataset().getAnalysisOptions()
                        .getDetectionOptions(IAnalysisOptions.SPERM_TAIL).getCannyOptions(), channel);
    ...
    // try {
    // if(this.get()){
    // firePropertyChange("Finished", getProgress(),
    // Constants.Progress.FINISHED.code());
    // } else {
    // firePropertyChange("Error", getProgress(),
    // Constants.Progress.ERROR.code());
    // }
