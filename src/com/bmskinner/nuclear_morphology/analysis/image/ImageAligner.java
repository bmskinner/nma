...
public class ImageAligner {
    ...
    private ImagePlus testImage;   // the image that will be offset to match the
                                   // reference
    ...
    /**
     * The max number of pixels to move in any direction. A value of 50 would
     * mean a range of -50 to 50 x and -50 to 50 y
     */
     ...
     *
     * @param staticImage
     *            the image that will be used as a reference
     * @param testImage
     *            the image that will be moved to best fit
     */
    ...
    public void run() {
        ...
        int interval = 5; // must be smaller than nuclear size to ensure some
                          // hits
        ...
        for (int x = xOffset - this.range; x < xOffset + this.range + 1; x += interval) {
            for (int y = yOffset - this.range; y < yOffset + this.range + 1; y += interval) {
                ...
                offsetImage(offsetImage, x, y); // need to use a copy of the
                                                // image
