...
public class Kuwahara_Filter implements PlugInFilter {
    ...
    public void filter(ImageProcessor ip, int size) {
        this.size = size;
        filter(ip);
    }
    ...
    public void filter(ImageProcessor ip) {
        ...
        for (int y1 = y1start - offset; y1 < y1start + height; y1++) {
            if ((y1 % 20) == 0)
                IJ.showProgress(0.7 * (y1 - y1start) / height);
        ...
        // new ImagePlus("Variance", new FloatProcessor(variance)).show(); //
        // ImageJ 1.35b or later
        int xbase2 = 0, ybase2 = 0;
        float var, min;
        for (int y1 = y1start; y1 < y1start + height; y1++) {
            if ((y1 % 20) == 0)
                IJ.showProgress(0.7 + 0.3 * (y1 - y1start) / height);
