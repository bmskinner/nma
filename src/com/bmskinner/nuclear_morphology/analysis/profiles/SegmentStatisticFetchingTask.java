...
import com.bmskinner.nuclear_morphology.analysis.AbstractStatisticFetchingTask;
...
public class SegmentStatisticFetchingTask extends AbstractStatisticFetchingTask {
	...
	private double[] getStatistics() throws Exception{
		...
		for(int i=0, j=low; j<high; i++, j++){
			...
			IBorderSegment segment = nuclei[j].getProfile(ProfileType.ANGLE, Tag.REFERENCE_POINT).getSegment(id);

			  double perimeterLength = 0;
			  if(segment!=null){
				  int indexLength = segment.length();
				  double fractionOfPerimeter = (double) indexLength / (double) segment.getTotalLength();
				  perimeterLength = fractionOfPerimeter * nuclei[j].getStatistic(PlottableStatistic.PERIMETER, scale);
			  }
