...
public abstract class HistogramDatasetCreator extends AbstractDatasetCreator<ChartOptions> {
	...
	protected HistogramDatasetCreator(@NonNull final ChartOptions o) {
		super(o);
	}
	...
	/**
	 * Calculate the minimum and maximum ranges in a list of datasets for the given
	 * stat type
	 * 
	...
	public static HistogramDataset createHistogramDatasetFromList(List<Double> list) {
		...
		if (!list.isEmpty()) {
			double[] values = list.stream().mapToDouble(Number::doubleValue).toArray();
	...
	public XYDataset createDensityDatasetFromList(List<Double> list, double binWidth)
		...
		double[] yData = yValues.stream().mapToDouble(Number::doubleValue).toArray();
