...
//									SingleDatasetResultAction.NO_FLAG, acceptor, segmentLatch).run();
...
//							new RefoldNucleusAction(selectedDatasets, acceptor, refoldLatch).run();
//						} catch (InterruptedException e) {
//							Thread.currentThread().interrupt();
//							return;
//						}
...
//							new ExportDatasetAction(selectedDatasets, acceptor, saveLatch).run();
//						} catch (InterruptedException e) {
//							Thread.currentThread().interrupt();
//							return;
//						}
...
//				return new ShellAnalysisAction(event.firstDataset(), acceptor);
//			}
//
...
//				return new ClusterAnalysisAction(event.firstDataset(), acceptor);
//			}
//
//			if (event.method().equals(DatasetEvent.MANUAL_CLUSTER)) {
//				LOGGER.fine("Manually clustering dataset");
//				return new ManualClusterAction(event.firstDataset(), acceptor);
//			}
//
//			if (event.method().equals(DatasetEvent.CLUSTER_FROM_FILE))
//				return new ClusterFileAssignmentAction(event.firstDataset(), acceptor);
//
//			if (event.method().equals(DatasetEvent.BUILD_TREE)) {
//				LOGGER.fine("Building a tree from dataset");
//				return new BuildHierarchicalTreeAction(event.firstDataset(), acceptor);
//			}
//
...
//					final CountDownLatch latch = new CountDownLatch(1);
//					new Thread(() -> {
//						new RunProfilingAction(selectedDatasets, SingleDatasetResultAction.NO_FLAG, acceptor, latch)
//								.run();
//					}).start();
//
	...
	public synchronized void saveRootDatasets() {
		...
		Runnable r = () -> {
			for (IAnalysisDataset root : DatasetListManager.getInstance().getRootDatasets()) {
				...
				new Thread(() -> {
					Runnable task = new ExportDatasetAction(root, acceptor, latch, false);
					task.run();
					try {
						latch.await();
					} catch (InterruptedException e) {
					...
					Runnable wrk = new ExportWorkspaceAction(DatasetListManager.getInstance().getWorkspaces(),
							acceptor);
					wrk.run();
				}).start();
			}
