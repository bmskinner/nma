...
public class ShellCount implements XmlSerializable {
	...
	public ShellCount(Element e) throws ComponentCreationException {
		...
		for (Element el : e.getChildren(XMLNames.XML_SIGNAL_SHELL_VALUES)) {
	...
	/**
	 * Add the given values to the key. Existing values are overwitten.
	 * 
	 * @param k
	 * @param values
	 */
	...
	/**
	 * Get the number of keys in the counter with the given aggregation level
	 * 
	 * @return
	 */
...
//        return results.values().stream().mapToLong(a->a[shell]).sum();
	...
	/**
	 * Fetch only the object keys that match the given aggregation level i.e
	 * {@link Aggregation.BY_NUCLEUS} will not fetch individual signal pixel values
	 * and {@link Aggregation.BY_SIGNAL} will not fetch whole nucleus pixel values
	 * 
	...
	public Set<ShellKey> keys(Aggregation agg) {
		switch (agg) {
		case BY_NUCLEUS:
			return results.keySet().stream().filter(k -> !k.hasSignal())
