...
/**
 * This shell result is designed to allow raw data to be exported from shell
...
public class DefaultShellResult implements IShellResult {
	...
	public DefaultShellResult(Element e) throws ComponentCreationException {
		...
		if (e.getChild(XMLNames.XML_SIGNAL_N_SHELLS) == null)
			throw new ComponentCreationException("No nShells child in element: " + e.toString());

		if (e.getChild(XMLNames.XML_SIGNAL_SHELL_SHRINK_TYPE) == null)
			throw new ComponentCreationException("No ShrinkType child in element: " + e.toString());

		if (e.getChildren(XMLNames.XML_SIGNAL_SHELL_COUNT).isEmpty())
			throw new ComponentCreationException(
					"No ShellCount children in element: " + e.toString());

		nShells = Integer.parseInt(e.getChildText(XMLNames.XML_SIGNAL_N_SHELLS));
		type = ShrinkType.valueOf(e.getChildText(XMLNames.XML_SIGNAL_SHELL_SHRINK_TYPE));

		for (Element el : e.getChildren(XMLNames.XML_SIGNAL_SHELL_COUNT)) {
			CountType c = CountType
					.valueOf(el.getAttributeValue(XMLNames.XML_SIGNAL_SHELL_COUNT_TYPE));
	...
	public Element toXmlElement() {
		...
		e.addContent(new Element(XMLNames.XML_SIGNAL_SHELL_SHRINK_TYPE).setText(type.toString()));

		for (Entry<CountType, ShellCount> entry : map.entrySet()) {
			e.addContent(entry.getValue().toXmlElement()
					.setAttribute(XMLNames.XML_SIGNAL_SHELL_COUNT_TYPE, entry.getKey().toString()));
		}
	 ...
	 */
	public void addShellData(@NonNull CountType countType, @NonNull ICell c, @NonNull Nucleus n,
			long @NonNull [] shellData) {
	 ...
	 * @param shellData the pixel intensity counts per shell
	 */
	public void addShellData(@NonNull CountType countType, @NonNull ICell cell,
			@NonNull Nucleus nucleus, @Nullable INuclearSignal signal, long @NonNull [] shellData) {
	 ...
	 * @return the proportion of signal in the shell in the range 0-1 for each
	 *         object
	 */
		...
		case DAPI: {
			return map.get(CountType.SIGNAL).keys(agg).stream()
					.mapToDouble(k -> {
						...
						long[] cnts = map.get(CountType.COUNTERSTAIN)
								.getPixelIntensities(k.componentKey());
	 ...
	 * @param norm  the normalisation to apply
	 * @param shell the shell to fetch
	 * @return the mean signal proportion (the mean of the values returned by
	 *         {@link getProportions()}
	 */
