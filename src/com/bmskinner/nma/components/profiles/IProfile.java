...
public interface IProfile extends Iterable<Integer>, XmlSerializable {
	...
	/**
	 * Get the fractional distance of the given index along the profile from zero to
	 * one.
	 * 
	...
	/**
	 * Get the minimum value in the profile.If there are multiple values at minimum,
	 * this returns the first only
	 * 
	 ...
	 * 
	 * <pre>
	 * Profile A    compared to    Profile B
	 * </pre>
	 * 
	 * <pre>
	 * 111222111                   111242110
	 ...
	 * </pre>
	 * 
	 * has a difference at one index of 2, and at another of 1. The sum of squares
	 * difference is 2^2 + 1^2 = 5. <br>
	 * The order of comparisons does not matter when profiles are different lengths,
	 * since the shorter profile is interpolated. That is,
	 * {@code shortProfile.absoluteSquareDifference(longProfile)} will yield the
	 * same result as {@code longProfile.absoluteSquareDifference(shortProfile)}.
	 * <p>
	 * However, when comparing multiple profiles it is advisable to normalise their
	 * lengths <b>before</b> running the square difference calculation.
	 * 
	 ...
	 * {@code 5, 10, 15, 20} starting from index 1 will become
	 * {@code 10, 15, 20, 5 }.
	 * 
	 * Setting a start of -1 will produce {@code 20, 5, 10, 15 }.
	 * 
	 * The offsets are reversible: {@code profile.startFrom(x).startFrom(-x)} will
	 ...
	 * E.g. smoothing with window size 2 will average across 5 points: 2 behind, the
	 * index being smoothed, and 2 ahead.
	 * 
	 ...
	 * Interpolates the profiles to the length of this profile. Finds the point of
	 * least difference, and return the offset. It will always return a positive
	 * value.
	 * 
	 * @param testProfile
	 ...
	 * defined range. Interpolates the profiles to the length of this profile. Finds
	 * the point of least difference, and return the offset. It will always return a
	 * positive value.
	 * 
	 * @param testProfile
	 * @return the offset to this profile that must be applied to match the test
	 ...
	 * points <i>windowSize</i> ahead and behind are checked. Each should be greater
	 * than the value before. One exception is allowed, to account for noisy data.
	 * 
	 * @param windowSize the number of indices ahead and behind to check
	 * @return a boolean profile with the indexes of local minima as true and all
	 *         other indexes false
	 */
	 ...
	 * must originate from an equivalent profile (i.e. have the same totalLength as
	 * the profile)
	 * 
	 ...
	 * size around each point. E.g. the profile 1-2-3-4-5 with a window size of 1
	 * will give (1-5=-4) + (2-1=1) = -3 for index 0
	 * 
	...
	/**
	 * Calculate the difference between each index and the following index. No
	 * smoothing or windowing is performed.
	 * 
	 * @return a profile with the derivative values
	 */
	IProfile calculateDerivative();
	 ...
	 * 
	 * @param divider the profile to divide by. Must be the same length as this
	 *                profile
	 * @return the new profile
	 */
	...
	/**
	 * Wrap the given index into the current profile For example, here is a profile
	 * of length 10 (indexes 0-9), with some indexes to be accessed beyond each end
	 * of the profile:
	 * <p>
	 * 
	 * <pre>
	 ...
	 * </pre>
	 * <p>
	 * These indexes need to be wrapped to their appropriate index in the profile,
	 * preserving the number of index steps.
	 * <p>
