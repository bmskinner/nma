 ...
 * </pre>
 * <p>
 * This is the number of unique indexes in the profile. In the profile above,
 * there are 10 unique indexes, and the profile length is thus 10.
 * 
 * A segment is a contiguous set of indices in a profile. Each segment has a
 * start index and an end index. For example:
 * <p>
 * 
 * <pre>
 * 0  3   7 9<br>---|---|--<br>    s0
 * </pre>
 * <p>
 * In the profile above, a segment boundary has been drawn at index 3 and index
 * 7. This results in two segments. Segment s0 has a start index of 3, and an
 * end index of 7. The length of the segment is calculated at the number of
 * profile indexes contained within the segment. For s0, this is 3-4-5-6-7, for
 * a length of 5.
 * <p>
 * A second feature of segments is that they can <i>wrap</i>. The profile has a
 * second segment, s1.
 * <p>
 * 
 * <pre>
 * 0  3   7 9<br>---|---|--<br>s1  s0  s1
 * </pre>
 * <p>
 * Segment s1 begins at the breakpoint at index 7, and continues to the end of
 * the profile. It then resumes at index 0 - wrapping around the end of the
 * profile - and continues to index 3. It therefore has a start index of 7 and
 * an end index of 3. The length of the segment is 7-8-9-0-1-2-3, for a length
 * of 7.
 * <p>
 * Note that the total length of the segments in the profile is longer than the
 * profile itself. The profile with length 10 has segments with combined length
 * 12 (5+7). The combined segment length is profileLength + nSegments. This is
 * because adjacent segments must share at least one breakpoint index. When
 * implementing this interface, it is important to bear this in mind, and
 * <b>not</b> allow patterns such as:
 * <p>
 * 
 * <pre>
 * 0  34  789<br>---||--||-<br>s1   s0  s1
 * </pre>
 * <p>
 * In the example above, segments have adjacent endpoints, not overlapping
...
public interface IProfileSegment extends XmlSerializable, Iterable<Integer> {
	static final Logger LOGGER = Logger.getLogger(IProfileSegment.class.getName());

	/**
	 * The smallest number of indexes in a segment. Setting greater than 1 to allow
	 ...
	 * 
	 * @return true if a segment with the given id is present within this segmnet
	 *         (recursive)
	 */
	...
	/**
	 * Get the index closest to the fraction of the way through the segment. For
	 * example in the segment below, requesting proportion 0.5 will return index 3:
	 * 
	 * <pre>
	 * 0 0.5 1<br>|-----|<br>0  3  6
	 * </pre>
	...
	/**
	 * Get the proportion of the given index along the segment from zero to one. For
	 * example in the segment below, requesting index 3 will return 0.5:
	 * 
	 * <pre>
	 * 0  3  6<br>|-----|<br>0 0.5 1
	 * </pre>
	...
	/**
	 * Get the name of the segment in the form "Seg_n" where n is the position in
	 * the profile
	 * 
	 * @return
	 */
	...
	/**
	 * Gets the index closest to the middle of the segment. If the segment length is
	 * odd, this will be the exact midpoint. If the segment length is even, this
	 * will be the lower of the two possible midpoint indexes. For example:
	 * <p>
	 * 
	 * <pre>
	 * Length 8   Length 7<br>0      7   0     6<br>|------|   |-----|<br>   4          3
	 * </pre>
	 * 
	 * @return
	 */
	 ...
	 * Since profiles wrap, this may return the distance within the segment, or it
	 * may return the distance passing outside the segment.
	 * 
	 * @param index
	 * @return
	 */
	...
	/**
	 * Get the distance within the segment of the given index to the start of the
	 * segment
	 * 
	 * @param index
	 * @return
	...
	/**
	 * Get the shortest distance of the given index to the end of the segment. Since
	 * profiles wrap, this may return the distance within the segment, or it may
	...
	/**
	 * Get the distance within the segment of the given index to the end of the
	 * segment
	 * 
	 * @param index
	 * @return
	...
	/**
	 * Get the next segment in the profile. That is, the segment whose start index
	 * is the end index of this segment
	 * 
	 * @return the segment
	 */
	...
	/**
	 * Get the previous segment in the profile. That is, the segment whose end index
	 * is the start index of this segment
	 * 
	 * @return the segment
	 */
	...
	/**
	 * Create a copy of the segment and its merge sources. The start and end indexes
	 * will be increased by the given value, and wrapping applied based on the
	 * profile length.
	 * 
	...
	/**
	 * Test the effect of new start and end indexes on the length of the segment.
	 * Use for validating updates.
	 * 
	...
	/**
	 * Test if the segment wraps (i.e contains index 0 as anything other than a
	 * start index). For example:
	 * <p>
	 * 
	 * <pre>
	 *     4      11   <br>####|------|###<br> s1         s1
	 * </pre>
	 * 
	 * The segment s1 above starts at 11 and ends at 4. It wraps because it contains
	 * 0.
	 * <p>
	 * 
	 * <pre>
	 ...
	 * </pre>
	 * 
	 * The segment s1 above starts at 0 and ends at 0. It wraps because the start
	 ...
	 * </pre>
	 * 
	 * The segment s1 above starts at 0 and ends at 4. It does not wrap because it
	 * only has a start point of 0.
	 * 
	 * 
	 * @return
	 */
	...
	/**
	 * Test if the segment contains the given index. Returns true if the index is
	 * the first index, last index, or any index in between.
	 * 
	...
	/**
	 * Update the segment to the given position. Also updates the previous and next
	 * segments.
	 * 
	 * @param startIndex the new start index
	 * @param endIndex   the new end index (inclusive)
	 * @throws SegmentUpdateException if the update cannot proceed. The exception
	 *                                message gives detail on why the update failed
	 */
	boolean update(int startIndex, int endIndex) throws SegmentUpdateException;

	/**
	 * Set the next segment in the profile. The segment must have a start index that
	 * is equal to the end index of this segment.
	 * 
	 * @param s
	 * @throws IllegalArgumentException if the segment start does not overlap this
	 *                                  segment end
	 */
	void setNextSegment(@NonNull IProfileSegment s);

	/**
	 * Set the previous segment in the profile. The segment must have an end index
	 * that is equal to the start index of this segment.
	 * 
	 * @param s
	 * @throws IllegalArgumentException if the segment end does not overlap this
	 *                                  segment start
	 */
	...
	/**
	 * Test if the given segment overlaps this segment. Formally, returns true if
	 * any indexes are shared in the segments except for the start and end indexes
	 * 
	 * @param seg
	...
	/**
	 * Test if the given segment overlaps this segment. Formally, returns true if
	 * any indexes are shared in the segments, including the start and end indexes.
	 * If you want to test that segments only overlap at the endpoints, use
	...
	/**
	 * Reverse the segment in the profile. Acts as if the profile has been entirely
	 * reversed
	 * 
	 * @return
	 */
	...
	/**
	 * Given a list of segments, link them together into a circle. Links start and
	 * end properly. Does not copy the segments.
	 * 
	 * @param list
	...
	static List<IProfileSegment> scaleSegments(@NonNull List<IProfileSegment> list, int newLength)
		...
		// Special case when the length does not change - do nothing
		if (list.get(0).getProfileLength() == newLength) {
			return list;
		}

		// Special case when there is only a single segment
		if (list.size() == 1) {
		...
		// In case the start index of the segment list is not zero
		int startIndex = list.get(0).getStartIndex();
		double startProp = (double) startIndex / (double) list.get(0).getProfileLength();
		int newStartIndex = (int) (list.get(0).getProfileLength() * startProp);
		startIndex = newStartIndex;

		// Does the list of segments cover an entire profile or do we e.g. have merge
		// sources covering only part of the profile?
		int totalSegLength = list.stream().map(IProfileSegment::length).reduce(0,
				Integer::sum);
		boolean isPartial = totalSegLength < list.get(0).getProfileLength();

		for (int i = 0; i < list.size(); i++) {
			IProfileSegment segment = list.get(i);
//			LOGGER.fine("Scaling " + segment);
			...
			// Ensure final segment end index matches the first segment start index if we
			// have a complete profile covered
			...
			if (segment.hasMergeSources()) {
				List<IProfileSegment> mergeSources = segment.getMergeSources();
				List<IProfileSegment> mergeScaled = scaleSegments(mergeSources, newLength);

				IProfileSegment m0 = mergeScaled.get(0);
				IProfileSegment m1 = mergeScaled.get(1);

				// Ensure the scaled merge sources endpoints still match the parent segment
				// after int trunction of doubles
				m0.update(newSeg.getStartIndex(), m0.getEndIndex());
	 ...
	 * @return true if the distance between start and end indexes is longer than
	 *         {@link IProfileSegment.MINIMUM_SEGMENT_LENGTH}
	 */
	...
	/**
	 * Test the length for new segments. Can a second segment be added to the
	 * profile?
	 * 
	 * @param start the segment start index
	 * @param end   the segment end index
	 * @param total the profile length
	 * @return true if the total distance minus the distance between start and end
	 *         indexes is longer than {@link IProfileSegment.MINIMUM_SEGMENT_LENGTH}
	 */
	...
	/**
	 * Test if a segment with the specified start and end indexes would contain the
	 * given index.
	 * 
