 ...
 * should be written within the run() method. To signal progress through the
 * task use a {@code fireProgressEvent()}. This will tell registered
 * {@code ProgressListener}s to update UI elements accordingly. Typically this
 * will be an {@link IAnalysisWorker}. Calling {@code fireProgressEvent()} with
 * no parameters indicates that the external worker should increase the progress
 * by one, where the worker is tracking the total amount of 'work' in the task.
 * Calling {@code fireProgressEvent(long l)} will tell the worker to update the
 * progress to the given value out of the total amount of 'work' in the task.
 * 
...
public abstract class AbstractAnalysisMethod
		implements IAnalysisMethod, ProgressListener {

	private static final Logger LOGGER = Logger.getLogger(AbstractAnalysisMethod.class.getName());
	...
	@Override
	public IAnalysisMethod then(@NonNull final IAnalysisMethod nextMethod) throws Exception {
		call();
		return nextMethod;
	}
	...
	/**
	 * Update the total number of steps in the task, and alert progress listeners.
	 * For example, can set progress bar lengths
	 * 
	 * @param totalProgress the total number of steps in the task
	 */
	@Override
	public final void fireUpdateProgressTotalLength(int totalProgress) {
	 ...
	 */
	@Override
	public final void fireIndeterminateState() {
		fireProgressEvent(new ProgressEvent(this, ProgressEvent.SET_INDETERMINATE, 0));
	}

	/**
	 * Fire a progress event to listeners, indicating one step of the task has been
	 * completed
	 */
	@Override
	public final void fireProgressEvent() {
	 ...
	 */
	@Override
	public final void fireProgressEvent(long stepsNewlyCompleted) {
		ProgressEvent e = new ProgressEvent(this, ProgressEvent.INCREASE_BY_VALUE,
				stepsNewlyCompleted);
		fireProgressEvent(e);
	}

	@Override
	public final void fireProgressEvent(ProgressEvent e) {
