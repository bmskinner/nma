...
public class TextFileAnalysisAction extends VoidResultAction {
	...
	private File coordinateFile = null;
	 ...
	 */
	public TextFileAnalysisAction(@NonNull ProgressBarAcceptor acceptor, @Nullable final File coordinateFile) {
	...
	public void run() {
		...
		if(dialog.isReadyToRun()) {
			...
			File detectionFile = options.getNucleusDetectionOptions().get().getFile(HashOptions.COORDINATE_LOCATION_FILE_KEY);
			if (!detectionFile.exists()) {
				cancel();
				return;
			}

			LOGGER.info("Detecting objects in '%s'".formatted(detectionFile.getName()));
			...
			File outputFolder = new File(detectionFile.getParent(), anTime
			...
			try {
				...
				LOGGER.log(Level.SEVERE, "Unable to run detection: %s".formatted(e.getMessage()));
				this.cancel();
			}
		} else {
			this.cancel();
	...
	private class TextFileAnalysisOptionsDialog extends SettingsDialog {
		...
		private void createUI() {
			...
			String outlineFileName = coordinateFile==null ? "Click here to choose coordinate file" : coordinateFile.getAbsolutePath();
			JTextField  outlineFileBox = new JTextField(outlineFileName);
			outlineFileBox.setEditable(false);
			outlineFileBox.setEnabled(false);
			outlineFileBox.addMouseListener(new MouseAdapter() {
				...
				public void mouseClicked(MouseEvent e) {
					...
					try {
						coordinateFile = new DefaultInputSupplier().requestFile(GlobalOptions.getInstance().getDefaultDir());
						options.getNucleusDetectionOptions().get().setFile(HashOptions.COORDINATE_LOCATION_FILE_KEY, coordinateFile);
						outlineFileBox.setText(coordinateFile.getAbsolutePath());
			...
			JTextField  landmarkFileBox = new JTextField("Click here to choose landmark file (optional)");
			landmarkFileBox.setEditable(false);
			landmarkFileBox.setEnabled(false);
			landmarkFileBox.addMouseListener(new MouseAdapter() {

				@Override
				public void mouseClicked(MouseEvent e) {
					try {
						File landmarkFile = new DefaultInputSupplier().requestFile(GlobalOptions.getInstance().getDefaultDir());
		...
		private void setDefaults() {
			...
			HashOptions nucleusOptions = new OptionsBuilder()
					.withValue(HashOptions.CHANNEL, ImageImporter.RGB_BLUE) 
