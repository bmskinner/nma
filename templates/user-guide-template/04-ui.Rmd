...
Preferences | Open the preferences dialog window to view and modify program settings. The settings are saved in the configuration file in the `.nma` directory in the user home directory.
...
Cell pixel intensity histogram | Calculate the [pixel intensities](#nuclear-signals-pixel-intensity-tab) within each cell for selected image channels
...
### Cell pixel intensities {#nuclear-signals-pixel-intensity-tab}

Displays a summary of nucleus pixel histogram measurements when created via 'Dataset > Add... > Cell pixel intensity histogram'. 

Pixel intensity is calculated as (1 * the number of pixels with 1 intensity) + (2 * the number of pixels with 2 intensity) + (3 * the number of pixels with 3 intensity) etc. to intensity 255). Only pixels within a nucleus are included (*i.e.* the measurements are made per-nucleus, not for the image as a whole). The measurements for nuclear signals are also made with respect to the entire area of the nucleus.

The ratio between pixel intensities in two image channels is presented as a log2 ratio, which centres on zero and makes deviations above and below zero equally sized.

The underlying raw pixel histogram data can be exported via 'Dataset > Export > Export nuclear measurements' by including the 'Include pixel histograms' option

```{r, out.width="100%", out.height="100%", fig.show='hold', fig.align='center', echo=F}
knitr::include_graphics("img/Nuclear signals_Cell pixel intensities.png")
```
...
The Venn diagrams tab compares overlaps in nuclei between up to 5 datasets. It is primarily designed to allow comparison of different child datasets after filtering or clustering.

```{r, out.width="100%", out.height="100%", fig.show='hold', fig.align='center', echo=F}
knitr::include_graphics("img/Comparisons_Venn diagrams.png")
```

The Pairwise overlaps tab compares overlaps in nuclei between datasets, presented as a table. It is primarily designed to allow comparison of different child datasets after filtering or clustering.

```{r, out.width="100%", out.height="100%", fig.show='hold', fig.align='center', echo=F}
knitr::include_graphics("img/Comparisons_Pairwise overlaps.png")
```

Detailed Venn presents the pairwise overlaps data in a pairwise format, with more detailed breakdown of the numbers of shared and unshared nuclei.
