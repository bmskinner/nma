...
Cell pixel intensity histogram | Calculate the pixel intensities within each cell for selected image channels
...
### Cell pixel intensities

Displays a summary of image pixel histogram measurements when created via 'Dataset > Add... > Cell pixel intensity histogram'. The display is a log2 ratio of total pixel intensities for each combination of image channels in the dataset. 

Total pixel intensity is calculated as (1 * the number of pixels with 1 intensity) + (2 * the number of pixels with 2 intensity) + (3 * the number of pixels with 3 intensity) etc. to intensity 255).

The ratio between two signals is given as a log2 ratio, which centres on zero and makes deviations above and below zero equally sized.

The raw pixel data can be exported via 'Dataset > Export > Export nuclear measurements' by including the 'Include pixel histograms' option
