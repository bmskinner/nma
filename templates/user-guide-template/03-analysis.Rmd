...
Filter poor edge detection | Use unusally sharp angles in profiles to exclude cells with sharp corners that are most likely due to inaccurate edge detection. Defaults to false.
...
Segment profiles | Should the angle profiles be segmented at local maxima and minima? This is the default, but it doesn't give extra information for all nuclei (e.g. round nuclei).

### Detected objects

This contains a table showing the area in pixels and circularity of all objects detected in the current image. Any objects that will be excluded by the size or circularity filters are in red. You can use this to help choose the best size and circularity filtering thresholds for your images.
...
Option | Action
-------|----------
Nuclear measurements | The measured values for each nucleus and normalised profiles
