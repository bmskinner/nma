...
Option | Action
-------|--------
New analysis > Use custom detection options  | [Create a new morphology analysis](https://bitbucket.org/bmskinner/nuclear_morphology/wiki/methods/Creating%20a%20new%20analysis)
New analysis > Use custom detection options  | Run an analysis on images using saved detection options
New workspace | Create a [workspace ](https://bitbucket.org/bmskinner/nuclear_morphology/wiki/data/Workspaces) to enable opening datasets in one batch
Open > Open dataset | [Open a saved .nmd dataset](https://bitbucket.org/bmskinner/nuclear_morphology/wiki/methods/Opening%20a%20saved%20dataset)
Open > Open workspace | Open a [workspace ](https://bitbucket.org/bmskinner/nuclear_morphology/wiki/data/Workspaces)
Save selected | Save the datasets currently selected in the populations panel
...
Colour palette | set the colour palette to use for colouring datasets and drawing segments in profiles. These swatches are:       `Regular` - a set of bright exciting colours; `No colour` - all colours are black; `Accessible` - a set of colours chosen to be distinguishable in three major types of color blindness: deutranopia, protanopia and tritanopia 
...
Task monitor | Toggle the task queue and memory and displays. The first shows the queue of tasks waiting to be performed (e.g. drawing charts, running analyses). The second shows the amount of memory being used by the program. Free memory is green. Memory reserved by the Java Virtual Machine is orange, and memory actually in use is red. The percentage displays the memory actually in use (the red section). Hover the mouse over the monitor to see the total available memory (or look at the log file).

### Dataset {#dataset-menu}

Option | Action
-------|-------
Add... | Submenu with options for adding new data to a dataset; see below
[Curate](https://bitbucket.org/bmskinner/nuclear_morphology/wiki/methods/Manually%20curate%20cells) | Go through cells one by one, and mark them as keep or reject. Desired cells are saved as a child dataset.
...
[Boolean](https://bitbucket.org/bmskinner/nuclear_morphology/wiki/methods/Boolean%20operations) | Perform boolean algebra between this dataset and another dataset.
Add to workspace... | Allows datasets to be added to an existing [workspace](https://bitbucket.org/bmskinner/nuclear_morphology/wiki/data/Workspaces)
Remove from workspace... | Allows datasets to be removed from a [workspace](https://bitbucket.org/bmskinner/nuclear_morphology/wiki/data/Workspaces)
...
Export... | Submenu with options for exporting data; see below
...
[Post-FISH mapping](https://bitbucket.org/bmskinner/nuclear_morphology/wiki/methods/Post-FISH%20remapping) | Assign pre-FISH nuclei to categories based on FISH image data
Warped signals | Begin signal warping for a root dataset with nuclear signals
...
Details are described [here](https://bitbucket.org/bmskinner/nuclear_morphology/wiki/methods/Exporting%20raw%20data)
...
The populations panel holds the list of datasets currently loaded, and allows sub-analyses to be run on them.
The width of the populations panel can be adjusted using the slider between the log panel and the populations panel.

![populations_panel.png](https://bitbucket.org/repo/eA585a/images/2366104558-populations_panel.png)
...
Info | Image
-----|------
Two types of dataset can be represented: root datasets and child datasets. A root dataset is the top-level dataset in the tree; it is not a child of any other dataset. Child datasets have a parent dataset; either a root dataset, or another child dataset. | ![root_and_child.png](https://bitbucket.org/repo/eA585a/images/3570275142-root_and_child.png)
The name of the dataset can be changed by double clicking the 'name' cell. If a proposed name exists in the panel already, the software will prompt you to choose a different name. | ![rename_collection.png](https://bitbucket.org/repo/eA585a/images/2617437421-rename_collection.png)
The colour of the dataset for multi-dataset displays (e.g. profiles, boxplots, consensus nucleus outlines) can be set by double clicking the colour cell. By default, a colour is selected based on the position of the dataset in the list of all selected datasets: the first is blue, the second yellow, the third green etc. The default colour pattern is the same as the segment pattern colours. The default colour is overridden if a colour has been explicitly set. The colour will be remembered if the dataset is saved before the program is closed. | ![change_dataset_colour.png](https://bitbucket.org/repo/eA585a/images/2827573362-change_dataset_colour.png)
...
Rotation and offsets can also be applied in exact amounts using the options at the bottom of the right-click menu: `Rotate by..` and `Offset...`

![consensus_popup_menu.png](https://bitbucket.org/repo/eA585a/images/751057011-consensus_popup_menu.png)
...
Scatter | Allows comparisons between two parameters, via the drop down lists. The Spearman's rank correlation coefficient for each selected dataset is given in the table to the left. The nuclei can be [filtered](https://bitbucket.org/bmskinner/nuclear_morphology/wiki/methods/Filter%20a%20collection) using the 'Filter visible' button. ![Nuclear_charts_scatter.png](img/Nuclear_charts_scatter.png)
...
Display and run shell analysis.
...
```{r, out.width="100%", out.height="100%", fig.show='hold', echo=F}
knitr::include_graphics("img/Clusters_panel_overview.png")
```

There are three ways to generate clusters:

- Manually
- Automatically using a clustering algorithm
- [Importing clusters created in another program](https://bitbucket.org/bmskinner/nuclear_morphology/wiki/methods/Manually%20assign%20clusters.md)

### Manual clustering

Allows nuclei to be manually assigned to clusters. Choose the number of clusters you want to make, and each nucleus will be displayed for you to put in a cluster.

### Automatic clustering

Allows nuclei to be assigned to clusters based on their shape, or other measurements. To run a new clustering analysis, click 'Cluster automatically'.

```{r, out.width="100%", out.height="100%", fig.show='hold', echo=F}
knitr::include_graphics("img/gifs/Cluster_automatic.gif")
```

The default clustering is on the profile angles within each nucleus. Checkboxes allow selection of other nucleus parameters.

Dimensionality reduction can be optionally applied before clustering. PCA and tSNE are provided; however, it is recommended that for full analysis you run these in your preferred stats package. An example of how to perform clustering and dimensionality reduction using R is shown in [this page](https://bitbucket.org/bmskinner/nuclear_morphology/wiki/methods/Running%20a%20tSNE%20in%20R).
...
Once cluster is complete, if hierarchical clustering was selected, a 'Show tree' button will be available. Clicking this button will launch the tree viewer.
...
If a dataset is a merge of other datasets, this panel displays the source datasets.

To merge datasets, select them in the [populations panel](#populations-panel), select `Dataset > Merge'. You will be asked to select a place to save the new merged dataset. The merge will run, and a new morphological analysis will be carried out on the merged dataset.

When a merged dataset is selected, the merges tab displays the analysis options for each of the merge sources. Matching values are highlighted in green for each row.
