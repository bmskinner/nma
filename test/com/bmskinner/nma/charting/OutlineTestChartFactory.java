...
public class OutlineTestChartFactory extends ChartFactoryTest {
	 ...
	 */
	public static JPanel generateOutlineChart(@NonNull IAnalysisDataset dataset, @NonNull ICell cell)
			throws InterruptedException {
		String title = new Exception().getStackTrace()[0].getMethodName();
		ChartOptions options = new ChartOptionsBuilder().setDatasets(dataset).setCell(cell)
				.addCellularComponent(cell.getPrimaryNucleus()).build();
		return makeChartPanel(new OutlineChartFactory(options).makeCellOutlineChart(), options, title, IS_FIXED_ASPECT);
	}

	public static void showOutlineChart(@NonNull IAnalysisDataset dataset, @NonNull ICell cell)
			throws InterruptedException {
	 ...
	 * @throws InterruptedException
	 */
	public static void generateOutlineChartsForAllCells(@NonNull IAnalysisDataset dataset, @Nullable String title)
			throws InterruptedException {
		if (title == null)
		...
		for (ICell cell : dataset.getCollection().getCells()) {
			ChartOptions options = new ChartOptionsBuilder().setDatasets(dataset).setCell(cell).build();
			JPanel outline = makeChartPanel(new OutlineChartFactory(options).makeCellOutlineChart(), options, title,
					IS_FIXED_ASPECT);
