...
public class ProfileChartFactoryTest extends ChartFactoryTest {
	...
	/**
	 * Create charts with a default range of options variables for the given
	 * datasets
	 * 
	 * @param datasets
	 * @throws InterruptedException
	...
	private void generateChartsforOptions(List<IAnalysisDataset> datasets, String title) throws InterruptedException {
		...
		for (OrientationMark tag : OrientationMark.values()) {
			ChartOptions options = new ChartOptionsBuilder().setDatasets(datasets).setLandmark(tag).setShowMarkers(true)
					.setNormalised(true).build();
		...
		for (ProfileAlignment p : ProfileAlignment.values()) {
			ChartOptions options = new ChartOptionsBuilder().setDatasets(datasets).setAlignment(p).setNormalised(false)
					.build();
		...
		for (ColourSwatch p : ColourSwatch.values()) {
			ChartOptions options = new ChartOptionsBuilder().setDatasets(datasets).setSwatch(p).build();
			panels.add(makeChartPanel(new ProfileChartFactory(options).createProfileChart(), options, "Swatch: " + p,
					IS_FIXED_ASPECT));
		}
		ChartOptions trueOptions = new ChartOptionsBuilder().setDatasets(datasets).setNormalised(true)
				.setAlignment(ProfileAlignment.LEFT).setLandmark(OrientationMark.REFERENCE).setShowMarkers(true)
				.setProfileType(ProfileType.ANGLE).setSwatch(ColourSwatch.REGULAR_SWATCH).setShowAnnotations(true)
				.setShowPoints(true).setShowXAxis(true).setShowYAxis(true).build();

		panels.add(makeChartPanel(new ProfileChartFactory(trueOptions).createProfileChart(), trueOptions, "All true",
				IS_FIXED_ASPECT));

		ChartOptions falseOptions = new ChartOptionsBuilder().setDatasets(datasets).setNormalised(false)
				.setAlignment(ProfileAlignment.LEFT).setLandmark(OrientationMark.REFERENCE).setShowMarkers(false)
				.setProfileType(ProfileType.ANGLE).setSwatch(ColourSwatch.REGULAR_SWATCH).setShowIQR(false)
				.setShowAnnotations(false).setShowPoints(false).setShowXAxis(false).setShowYAxis(false).build();
	...
	public void testSingleNucleusProfile() throws Exception {
		...
		ChartOptions trueOptions = new ChartOptionsBuilder().setDatasets(d).setCell(c).setNormalised(true)
				.setAlignment(ProfileAlignment.LEFT).setLandmark(OrientationMark.REFERENCE).setShowMarkers(true)
				.setProfileType(ProfileType.ANGLE).setSwatch(ColourSwatch.REGULAR_SWATCH).setShowAnnotations(true)
				.setShowPoints(true).setShowXAxis(true).setShowYAxis(true).build();
		...
		ChartOptions falseOptions = new ChartOptionsBuilder().setDatasets(d).setCell(c).setNormalised(false)
				.setAlignment(ProfileAlignment.LEFT).setLandmark(OrientationMark.REFERENCE).setShowMarkers(false)
				.setProfileType(ProfileType.ANGLE).setSwatch(ColourSwatch.REGULAR_SWATCH).setShowAnnotations(false)
				.setShowPoints(false).setShowXAxis(false).setShowYAxis(false).build();
