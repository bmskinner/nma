...
public class DatasetStatsExporterTest {
	...
	public void testSegmentsInterpolated() throws Exception {
		...
		while(d.getCollection().getProfileCollection().getSegmentContaining(0).length()>IBorderSegment.MINIMUM_SEGMENT_LENGTH*0.7){
			IBorderSegment seg = d.getCollection().getProfileCollection().getSegmentContaining(0);
		...
		assertTrue(d.getCollection().getProfileCollection().getSegmentContaining(0).length()<IBorderSegment.MINIMUM_SEGMENT_LENGTH);
